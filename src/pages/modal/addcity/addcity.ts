...
export class AddCity {
  ...
  addCity(cityName, cityId){
    if(localStorage.getItem("cityList")){
      let cityList = JSON.parse(localStorage.getItem("cityList"));
      cityList.push({"cityName": cityName, "cityId": cityId});
      localStorage.removeItem("cityList");
      localStorage.setItem("cityList", JSON.stringify(cityList));
      this.navCtrl.push(TabsPage, {tabSelect: "1"})
    }else{
      let cityList = [{"cityName": cityName, "cityId": cityId}];
      localStorage.removeItem("cityList");
      localStorage.setItem("cityList", JSON.stringify(cityList));
      this.navCtrl.push(TabsPage, {tabSelect: "1"});
    }
  ...
  * */
  searchCity(){
    if(this.cityName){
      //汉语转拼音
      let pinyin = this.convertPinyin(this.cityName);
      let params = new URLSearchParams();
      params.set("location", pinyin);
      this.http.get("/cityid", params).then(res => {
        if(res["results"].length > 0){
          this.preCityList = res["results"];
          this.showFlag = true;
        }else{
          this.showFlag = false;
        }
